...
const Villages = () => {
  ...
  const getPredefinedColor = index => {
    // Use modulo operator to cycle through colors if the index exceeds the length of the array
    return predefinedColors[index % predefinedColors.length];
  };

  const renderItem = data => {
    const villageName = language === 'en' ? data.item.villageE : data.item.villageG;
    const colorIndex = data.index % predefinedColors.length; // Calculate the index of the color based on the item index
  ...
  return (
    ...
    //     <Text style={styles.text}>{villagesData.villageE}</Text>
    //     <Text style={styles.text1}>{villagesData.villageE}</Text>
    //   </View>
    //   ) : (
    //     <View style={styles.blankcontainer}>
    //       <Text style={styles.blank}>{t('nosearchdatafound')}</Text>
